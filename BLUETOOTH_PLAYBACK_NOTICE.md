...
When **reviewing recordings** in Recall, we recommend using **computer speakers** or **wired headphones** rather than Bluetooth headphones for accurate playback.
...
1. **Recall records at 48kHz** (professional audio standard)
...
- ❌ **Reviewing Recall recordings** - Use wired/speakers instead
...
### This is Not a Recall Bug

The issue is in **macOS's Bluetooth audio stack**, not in Recall's recording engine.
...
**A**: This is a macOS/Bluetooth limitation, not a Recall bug. We've correctly recorded at 48kHz.
...
**A**: Yes! Recall now shows a warning when Bluetooth headphones are active during playback.
...
**Applies To**: Recall on macOS (originally documented for Meetily v0.0.5+)
