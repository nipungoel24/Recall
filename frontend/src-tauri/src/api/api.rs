...
/// Lightweight meeting metadata for calendar views, returned by
/// `api_get_meetings_by_date_range`. Includes an optional meeting duration
/// derived from transcript timing metadata (no transcript text is fetched).
...
#[tauri::command]
pub async fn api_get_meetings_by_date_range<R: Runtime>(
    _app: AppHandle<R>,
    state: tauri::State<'_, AppState>,
    start_date: String,
    end_date: String,
    ...
    log_info!(
        "api_get_meetings_by_date_range called with start_date: {}, end_date: {}, auth_token: {}",
        start_date,
        end_date,
        auth_token.is_some()
    );

    // Validate RFC 3339 inputs up-front so the contract fails loudly instead of
    // silently returning an empty range when julianday() cannot parse them.
    for (label, value) in [("start_date", &start_date), ("end_date", &end_date)] {
        if chrono::DateTime::parse_from_rfc3339(value).is_err() {
            return Err(format!(
                "{} must be a valid RFC 3339 timestamp, got: {}",
                label, value
            ));
        }
    }
    ...
    if start_date >= end_date {
        return Err("start_date must be strictly before end_date".to_string());
    }

    let pool = state.db_manager.pool();

    match MeetingsRepository::get_meetings_by_date_range(pool, &start_date, &end_date).await {
        Ok(meetings) => {
            log_info!(
                "Successfully got {} meetings between {} and {}",
                meetings.len(),
                start_date,
                end_date
            );
        ...
        Err(e) => {
            log_error!("Error getting meetings by date range: {}", e);
            Err(format!("Failed to get meetings by date range: {}", e))
        }
    }
}
