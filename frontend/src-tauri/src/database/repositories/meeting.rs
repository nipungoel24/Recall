use crate::api::{CalendarMeeting, MeetingDetails, MeetingTranscript};
...
impl MeetingsRepository {
    ...
    /// Get lightweight meeting metadata for a date range.
    ///
    /// `start_date` and `end_date` are RFC 3339 instants (e.g. "2026-08-15T04:00:00Z").
    /// The range is half-open: `created_at >= start_date AND created_at < end_date`.
    ///
    /// The caller is responsible for converting local day boundaries into UTC instants,
    /// so "August 15" means August 15 in the *user's* local timezone. Stored timestamps
    /// can use either RFC 3339 ("T"/"Z") or sqlx's space-separated format, so
    /// comparisons go through SQLite's `julianday()` to normalize both.
    ///
