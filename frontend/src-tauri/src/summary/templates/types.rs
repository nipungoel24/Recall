...
///
/// Used by the UI to distinguish user-created templates (which can be
/// edited and deleted) from bundled/built-in templates (which are
/// read-only).
...
pub enum TemplateSource {
    ...
    Builtin,
    /// Shipped as a JSON file in the app resource directory.
    Bundled,
    /// User-created JSON file in the application data directory.
    Custom,
}

impl TemplateSource {
    /// Lowercase string representation (matches the serde `rename_all`).
