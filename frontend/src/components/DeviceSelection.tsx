...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError('Unable to load audio devices. The app may not have permission to access them.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-foreground transition-colors hover:bg-muted disabled:pointer-events-none disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
      ...
      {error && (
        <div className="p-3 text-sm text-destructive bg-destructive/5 border border-destructive/40 rounded-md">
          {error}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-muted-foreground">
              No microphones detected. Connect a microphone or grant microphone access, then refresh.
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-muted-foreground">
              No system audio devices detected. You can still record with your microphone.
            </p>
          )}
      ...
      <div className="text-xs text-muted-foreground space-y-1">
        <p>• <strong className="text-foreground">Microphone:</strong> Records your voice</p>
        <p>• <strong className="text-foreground">System Audio:</strong> Records computer audio</p>
      </div>
    </div>
  );
}